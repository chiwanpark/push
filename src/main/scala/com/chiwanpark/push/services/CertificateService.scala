...
trait CertificateService extends WebService with DatabaseService {
  private val log = Logger(LoggerFactory.getLogger(classOf[CertificateService]))

  def getCertificate = path(IntNumber) { id =>
    get {
      val query = CertificateQuery.selectById(id)
      onComplete(db.run(query)) {
        case Success(result) =>
          if (result.length > 0) {
            complete(HttpResponse(entity = result.head.toJson.compactPrint))
          } else {
  ...
  def getToken = path(IntNumber / "token") { id =>
    get {
      val query = CertificateQuery.selectById(id)
